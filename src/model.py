...
def extract_T5_model_pl(path, learning_rate, new_out=None, **kwargs):
    ...
    if model_dict["lm_head.weight"].shape[0] != model.model.lm_head.weight.shape[0]:
        model.model.set_output_embeddings(
            nn.Linear(
                model.model.config.d_model,
                model_dict["lm_head.weight"].shape[0],
                bias=True if "lm_head.bias" in model_dict else False,
            )
        )
...
def extract_T5_model(path, learning_rate, new_out=None, **kwargs):
    ...
    if (
        model_dict["model.lm_head.weight"].shape[0]
        != model.model.lm_head.weight.shape[0]
    ):
        model.model.set_output_embeddings(
            nn.Linear(
                model.model.config.d_model,
                model_dict["model.lm_head.weight"].shape[0],
                bias=True if "model.lm_head.bias" in model_dict else False,
            )
        )
...
class T5Chem(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.cov_binder == "v1":
            accuracy_smiles, accuracy_start = self.test_cov_binder(
                batch, batch_idx, legacy=False
            )
            self.log(
                "accuracy_smiles",
                accuracy_smiles,
            ...
            accuracy_smiles, accuracy_pos = self.test_cov_binder_v2(
                batch, batch_idx, legacy=False
            )
            self.log(
                "accuracy_smiles",
    ...
    def test_cov_binder_v2(self, batch, batch_idx, legacy):
        ...
        if legacy:
            ...
            outputs = self.model(
                **batch
            )  # teacher forcing just doesn't as well for testing
            _, outputs = torch.max(F.softmax(outputs.logits, dim=-1), dim=-1)
            outputs = self.silence_noise_after_end(
                outputs.clone(),
                self.tokenizer.eos_token_id,
                self.tokenizer.pad_token_id,
            )
        accuracy_smiles, accuracy_start = self.collect_indices_smiles_v2(
            outputs.cpu(), labels.cpu()
        )
        return accuracy_smiles, accuracy_start

    def collect_indices_smiles_v2(
        self, output, labels, starting_token_idx=202, ending_token_idx=201
    ):
        # Find the indices of the start and end tokens
        start_indices, end_indices = self.validate_starting_ending_index(
            output, starting_token_idx, ending_token_idx
        )

        output_extracted_tokens = [
            output[i, start : end + 1].tolist()
            for i, (start, end) in enumerate(zip(start_indices, end_indices))
        ]
        output_extracted_pos = [
            output[i, :start].tolist()
            for i, (start, end) in enumerate(zip(start_indices, end_indices))
        ]
        label_start_indices, label_end_indices = self.validate_starting_ending_index(
            labels, starting_token_idx, ending_token_idx
        )
        labels_extracted_tokens = [
            labels[i, start : end + 1].tolist()
            for i, (start, end) in enumerate(
                zip(label_start_indices, label_end_indices)
            )
        ]
        label_extracted_pos = [
            labels[i, :start].tolist()
            for i, (start, end) in enumerate(zip(start_indices, end_indices))
        ]
        ...
        count = 0
        for pred_smiles, label_smiles in zip(
            output_extracted_tokens, labels_extracted_tokens
        ):
            pred_smiles = torch.tensor(pred_smiles[1:-1])
            label_smiles = torch.tensor(label_smiles[1:-1])
            if (
                pred_smiles.shape == label_smiles.shape
                and torch.eq(pred_smiles, label_smiles).all()
            ):
                count += 1
                ...
                if len(pred_smiles) == 0 and len(label_smiles) == 0:
                    ...
                    len(pred_smiles) == 0 or len(label_smiles) == 0
                ):  # Empty set on either the label or pred side!
                    pass
                    ...
                    try:
                        if Chem.CanonSmiles(pred_smiles) == Chem.CanonSmiles(
                            label_smiles
                        ):
                            count += 1
                    except:
    ...
    def _get_model_predictions(self, batch, legacy: bool) -> torch.Tensor:
        """
        Generates model predictions based on the legacy flag.

        Args:
            batch: The input batch containing input_ids and attention_mask.
            legacy: Boolean flag indicating whether to use legacy generation.

        Returns:
            A tensor containing the model's predictions.
        """
        if legacy:
            outputs = self.model.generate(
                input_ids=batch.input_ids,
            ...
            # Legacy generation includes a leading token (e.g., decoder_start_token_id)
            # that isn't part of the sequence to be evaluated. Remove it if present.
            if outputs.shape[1] > 0:
                return outputs[:, 1:]
            return outputs
        else:
            # Non-legacy path: use model's forward pass and argmax, then clean up generated tokens after EOS.
            # The comment "teacher forcing just doesn't as well for testing" was in the original code,
            # implying that using model's raw output logits is preferred over generate for eval here.
            model_outputs = self.model(**batch)
            _, outputs = torch.max(F.softmax(model_outputs.logits, dim=-1), dim=-1)
            outputs = self.silence_noise_after_end(
                outputs.clone(),
                self.tokenizer.eos_token_id,
                self.tokenizer.pad_token_id,
    ...
    def test_cov_binder(self, batch, batch_idx, legacy):
        """
        Tests covalent binder prediction accuracy.
        Prepares labels, generates model predictions according to the legacy flag,
        and then computes SMILES and start position accuracy.
        """
        labels = batch["labels"].clone().detach()
        labels[labels == -100] = 3  # Replace ignore_index with pad_token_id

        outputs = self._get_model_predictions(batch, legacy)

        accuracy_smiles, accuracy_start = self.collect_indices_smiles(
            outputs.cpu(),
            labels.cpu(),
            # starting_token_idx and ending_token_idx in collect_indices_smiles will use their defaults
        )
        return accuracy_smiles, accuracy_start
    ...
    def silence_noise_after_end(self, outputs, eos_token_id=2, pad_token_id=3):
        for i in range(outputs.size(0)):
            ...
            if (
                indices[0].nelement() > 0
            ):  # Check if there is at least one occurrence of 2
    ...
    def validate_starting_ending_index(
        self, output: torch.Tensor, starting_token_idx: int, ending_token_idx: int
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Validates and finds the start and end token indices for each sequence in a batch.

        For each sequence:
        - Finds the first occurrence of `starting_token_idx`.
        - Finds the first occurrence of `ending_token_idx` that comes *after* the found `starting_token_idx`.
        - If `starting_token_idx` is not found, its index defaults to 2.
        - If `ending_token_idx` is not found after `starting_token_idx` (or if `starting_token_idx` itself was not found),
          its index defaults to 1.

        Args:
            output: A 2D tensor of token IDs (batch_size, sequence_length).
            starting_token_idx: The token ID for the start token.
            ending_token_idx: The token ID for the end token.

        Returns:
            A tuple of two 1D tensors:
            - final_start_indices: Tensor containing the start index for each sequence.
            - final_end_indices: Tensor containing the end index for each sequence.
        """
        batch_size = output.shape[0]
        final_start_indices = torch.zeros(
            batch_size, dtype=torch.long, device=output.device
        )
        final_end_indices = torch.zeros(
            batch_size, dtype=torch.long, device=output.device
        )

        default_start_if_missing = 2
        default_end_if_missing = 1

        for i in range(batch_size):
            sequence = output[i]

            # Find all occurrences of start and end tokens in the current sequence
            start_occurrences = (sequence == starting_token_idx).nonzero(as_tuple=True)[
                0
            ]
            end_occurrences = (sequence == ending_token_idx).nonzero(as_tuple=True)[0]

            current_start_idx = -1
            current_end_idx = -1

            if start_occurrences.numel() > 0:
                current_start_idx = start_occurrences[
                    0
                ].item()  # Take the first start token

                if end_occurrences.numel() > 0:
                    # Find the first end token that is after the current_start_idx
                    valid_end_occurrences = end_occurrences[
                        end_occurrences > current_start_idx
                    ]
                    if valid_end_occurrences.numel() > 0:
                        current_end_idx = valid_end_occurrences[0].item()

            # Assign indices or defaults
            if current_start_idx != -1:
                final_start_indices[i] = current_start_idx
            else:
                final_start_indices[i] = default_start_if_missing

            if (
                current_end_idx != -1
            ):  # This implies current_start_idx was also found and valid
                final_end_indices[i] = current_end_idx
            else:
                # If start was found, but end was not (or not valid after start), end gets default.
                # If start was NOT found, end also gets default.
                final_end_indices[i] = default_end_if_missing

        return final_start_indices, final_end_indices

    def collect_indices_smiles(
        self, output, labels, starting_token_idx=202, ending_token_idx=201
    ):
        # The 'output' tensor should already be processed (e.g., leading token removed if necessary)
    ...
    def test_fwdrxn_step(self, batch, batch_idx, legacy):
        if legacy:
            temp_outputs = self.model.generate(
                batch.input_ids, max_length=300
            )  # TODO Investigate this to see if this makes sense.
        else:
    ...
    def prepare_for_accuracy(self, batch, temp_outputs):
        ...
        outputs = torch.nn.functional.pad(
            outputs, (0, max_length - outputs.shape[-1]), mode="constant", value=3
        )
        labels = torch.nn.functional.pad(
            labels, (0, max_length - labels.shape[-1]), mode="constant", value=3
        )
    ...
    def standize(self, outputs):
        ...
        outputs = outputs
        seq_indices = torch.full(
            (outputs.shape[0],), outputs.shape[-1], dtype=torch.long
        )
        ...
        results = self.tokenizer(
            new_outputs, padding=True, truncation=True, return_tensors="pt"
        )
        return results.input_ids.to(return_device)

    def accuracy(self, preds, labels):
        match_list = torch.tensor(
            [torch.equal(preds[i], labels[i]) for i in range(preds.shape[0])]
        )
    ...
    def configure_optimizers(self):
        ...
        scheduler = get_constant_schedule(
            optimizer
        )  # TODO switch scheduler to number of training steps
...
class T5ChemEsmAlign(T5Chem):
    ...
    def training_step(self, batch, batch_idx):
        outputs = self.model(**batch["t5chem"]).encoder_last_hidden_state.reshape(
            -1, self.model.config.d_model
        )
        esm_outputs = self.esm(**batch["esm"]).last_hidden_state.reshape(
            -1, self.model.config.d_model
        )
        loss = self.cosine_loss(
            outputs, esm_outputs, torch.tensor([1], device=outputs.device)
        )
        self.log(
            "cosine_loss",
            loss,
...
class T5PropertyRegression(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        if checkpoint_path is not None:
            self.model = extract_T5_model(
                checkpoint_path, learning_rate, 2 if loss == "kl" else 1, **kwargs
            )
        else:
        ...
        if loss == "kl" and self.model.lm_head.weight.shape[0] != 2:
            self.model.set_output_embeddings(
                nn.Linear(self.model.config.d_model, 2 * num_classes)
            )  # Method Call (change LM Head)
        elif loss == "mse" and self.model.lm_head.weight.shape[0] != 1:
            self.model.set_output_embeddings(
                nn.Linear(self.model.config.d_model, num_classes)
            )
        ...
        if "posaccuracy" in kwargs:
            self.pos_accuracy = PosAccuracy().to(
    ...
    def forward(
        ...
        outputs = self.model.forward(
            inputs["input_ids"], inputs["attention_mask"], inputs["decoder_input_ids"]
        )
        # make soft labels with inputs["labels"]
        if self.loss_type == "kl":
            soft_labels = nn.LogSoftmax(dim=-1)(
                einops.rearrange(outputs.logits.squeeze(1), "b (c v) -> b c v ", v=2)
            )  # B, 123, 2
            smoothed_labels = torch.stack(
                [1 - inputs["labels"], inputs["labels"]], dim=-1
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            if self.scaler:
                logits, batch_labels = self.unscale(
                    logits, batch["labels"].unsqueeze(-1)
                )
            else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.pos_accuracy:
            ...
            logits = logits.view(protein_length_shape).to(
                batch["protein_length"].device
            )
            batch_labels = batch_labels.view(protein_length_shape).to(
                batch["protein_length"].device
            )
            self.pos_accuracy(
    ...
    def on_test_epoch_end(self) -> None:
        ...
        if self.graph:
            ...
            tensorboard.add_figure(
                "test_prediction", figure, global_step=self.current_epoch
            )
            if self.save_fig:
                figure.savefig(
                    f"{self.logger.log_dir}/t5biochem_test_data_{self.extra if self.extra is not None else str()}.png"
                )
        if self.write:
            ...
            df = pd.DataFrame(
                {
                    "Pred": self.logits_coll.cpu().numpy().squeeze(),
                    "Target": self.labels_coll.cpu().numpy().squeeze(),
                }
            )
        ...
        if self.schedule is not None:
            if self.schedule == "warmup":
                scheduler = get_constant_schedule_with_warmup(
                    optimizer, num_warmup_steps=1446
                )
                ...
                scheduler = get_linear_schedule_with_warmup(
                    optimizer, num_warmup_steps=1446, num_training_steps=50000
                )
        else:
            scheduler = get_constant_schedule(
                optimizer
    ...
    def unscale(self, pred, target, train=False):
        ...
        target = torch.tensor(
            self.scaler.inverse_transform(target.detach().cpu().numpy())
        )
        return pred, target
...
class T5ESMClassification(T5Classification):
    ...
    def forward(self, **batch):
        esm_embeddings = self.esm(
            batch["esm_input_ids"], batch["esm_attention_mask"]
        ).last_hidden_state
        outputs = self.model(
            batch["input_ids"],
            batch["attention_mask"],
            decoder_inputs_embeds=esm_embeddings,
            decoder_attention_mask=batch["esm_attention_mask"],
        )
        loss_fn = nn.BCEWithLogitsLoss()
...
class ESMGoTerm(pl.LightningModule):
    def __init__(self, learning_rate, num_classes=489, lora=False, **kwargs) -> None:
        super().__init__()
        self.esm = EsmForSequenceClassification.from_pretrained(
            "facebook/esm2_t30_150M_UR50D", device_map="auto", num_labels=489
        )
    ...
    def forward(self, batch):
        # loss_fn = nn.BCELoss()
        outputs = self.esm(
            input_ids=batch["esm_input_ids"],
            attention_mask=batch["esm_attention_mask"],
            labels=batch["labels"],
        )
...
class T5ESMRegression(
    T5PropertyRegression
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            scaler_path, checkpoint_path, learning_rate, num_classes, lr_free, **kwargs
        )
        self.esm = EsmModel.from_pretrained("facebook/esm2_t30_150M_UR50D")
        # freeze all the parameters in esm
        if "freeze" in kwargs:
    ...
    def forward(
        ...
        esm_embeddings = self.esm(
            inputs["esm_input_ids"], inputs["esm_attention_mask"]
        ).last_hidden_state
        ...
        outputs = self.model(
            ...
            decoder_inputs_embeds=esm_embeddings,
            decoder_attention_mask=inputs["esm_attention_mask"],
        )  # Check on decoder attn mask
        # make soft labels with inputs["labels"]
    ...
    def __init__(self, model_path_dir, **kwargs) -> None:
        ...
        for model_path in model_paths.values():
            ...
            model_state_dict_path = os.path.join(
                model_path, "checkpoints", "model.model.pt"
            )
            if not os.path.isfile(model_state_dict_path):
                assert os.path.isfile(f"{model_path}/checkpoints/last.ckpt"), (
                    f"Model checkpoint not found at {model_path}/checkpoints/last.ckpt"
                )
                torch.save(
                    torch.load(
                        f"{model_path}/checkpoints/last.ckpt", map_location="cpu"
                    )["state_dict"],
                    model_state_dict_path,
                )
            model_config["model"]["init_args"]["checkpoint_path"] = (
                model_state_dict_path
            )
            model = (
                T5PropertyRegression(
                    **model_config["model"]["init_args"],
                    **model_config["model"]["dict_kwargs"],
                )
                .cuda()
                .eval()
            )
    ...
    def forward(self, **batch):
        ...
        with torch.no_grad():
            ...
            for model in self.models:
                ...
                preds, _ = model.unscale(
                    unscaled_output.unsqueeze(0), batch["labels"].unsqueeze(0)
                )
